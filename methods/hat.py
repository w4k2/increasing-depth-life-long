...
#             momentum=self.optimizer.defaults['momentum'],
...
#         self.r = len(self.adapted_dataset)
#         self.i = 0
...
#         self.s = (self.smax-1/self.smax)*self.i/self.r+1/self.smax

#     def _before_update(self, thres_cosh=50, **kwargs):
#         super()._before_update(**kwargs)
#         task_id = self.experience.current_experience
...
#         for n, p in self.model.named_parameters():
#             if n.startswith('e'):
#                 num = torch.cosh(torch.clamp(self.s*p.data, -thres_cosh, thres_cosh))+1
...
#     def _after_update(self, thres_emb=6, **kwargs):
#         super()._after_update(**kwargs)
#         for n, p in self.model.named_parameters():
#             if n.startswith('e'):
...
#         # Activations mask
#         task = torch.LongTensor([task_id]).to(self.device)
